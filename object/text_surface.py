...
class TextSurface(Object):
    ...
    def _prepare(self):
        ...
        max_text_width = (
            self.window_size[0]
            - self.text.padding[1] - self.text.padding[3]
            - self.text.outline.width * 2
            - self.text.margin[1] - self.text.margin[3]
        )
        ...
        for line in lines:
            ...
            if bg_surf:
                self.background_surfaces.append(bg_surf)
            if txt_surf:
    ...
    def render_text_with_outline(self, text, text_color, outline):
        ...
        for char in text:
            ...
            if char_is_emoji == current_is_emoji:
                ...
                parts.append((buffer, font_emoji if current_is_emoji else font_normal))
                buffer = char
                current_is_emoji = char_is_emoji
        ...
        parts = [(p, f) for p, f in parts if p.strip() and p != '️']
        if not parts:
            return None, None  # Rien à rendre

        text_width = sum(f.size(p)[0] for p, f in parts)
        text_height = max(f.size(p)[1] for p, f in parts)
        ...
        # Texte par ligne
        text_surface = pygame.Surface((surf_width, surf_height), pygame.SRCALPHA)
        x_cursor = outline.width + self.text.padding[3]
        for part, font in parts:
            if part.strip() == '':
                continue  # Évite crash sur espace seul
                ...
                for dx in [-outline.width, 0, outline.width]:
                    for dy in [-outline.width, 0, outline.width]:
                        if dx != 0 or dy != 0:
                            ...
                            text_surface.blit(outline_text, (text_x + dx, text_y + dy))

            text_surface.blit(part_surface, (text_x, text_y))
    ...
    def is_emoji(self, char):
        code = ord(char)
        return (
            code >= 0x1F300 or 0x2600 <= code <= 0x27BF
        )
    ...
    def _draw_surface(self, screen):
        ...
        if self.surface_title is not None:
            screen.blit(
                self.surface_title,
                (self.position.x + self.title.padding[1], self.position.y + self.title.padding[3])
            )

        for i, bg in enumerate(self.background_surfaces):
            x = self.position.x
            if "H" in self.position.justify:
        ...
        for i, txt in enumerate(self.surfaces):
            x = self.position.x
            if "H" in self.position.justify:
