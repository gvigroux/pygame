...
class Ball(Object):
    ...
    def __init__(self, context, vbo, json, window_size,amount =1, i = 1):
        ...
        self.position   = np.array(self.config("position", [random.uniform(-0.2, 0.2), random.uniform(-0.2, 0.2)]), dtype='f4')
        self.velocity   = np.array(self.config("velocity", [random.uniform(-0.01, 0.01), random.uniform(-0.01, 0.01)]), dtype='f4')
        ...
        ctx.arc(self.size // 2, self.size // 2, self.radius, 0, 2 * math.pi)
        ctx.fill()
    ...
    def collision_zone(self, arc):
        ...
        touch = ( distance >= (arc.radius - arc.width - self.radius) )
        if( not touch ):
            return None  
    ...
    def draw(self):
        ...
        Ball.prog['alpha'].value = self.alpha(30)
        self.texture.use()
